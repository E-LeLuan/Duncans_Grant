...
printf("\nThis is the version of EYEDIST that writes a file with two lines\nfor each subject, each containing all the FF durations, one for conditions\n1, 2, 5, and 6, and one for conditions 3, 4, 7, and 8.\nWhat do you want to call this file? ");
while((distrib = fopen(gets(file),"w")) EQ NULL)
	printf("\nBad file name, try again\n");
	fprintf(printer,"Distribution file name %s\n",distrib);
...
for(aptr=0;aptr<48;aptr++)		/* Kludgy stuff for Shelia, zero arrays */
	{
	distribs[0][aptr] = 0;
	distribs[1][aptr] = 0;
	}
dptr1=0;				/* zero global pointer too */
dptr2=0;
...
/* WRITE OUT INDIVID S DISTRIB ARRAYS */
for(aptr1=0;aptr1<=dptr1;aptr1++)
	fprintf(distrib,"%5d",distribs[0][aptr1]);
fprintf(distrib,"\n");
for(aptr1=0;aptr1<=dptr2;aptr1++)
	fprintf(distrib,"%5d",distribs[1][aptr1]);
fprintf(distrib,"\n");
...
/* Kludge to pick off good times */
		if(cptrptr == 0 || cptrptr == 1 || cptrptr == 4 || cptrptr == 5)
			distribs[0][dptr1++] = temptime;
		else
			distribs[1][dptr2++] = temptime;
		}
	else if(chatter)
