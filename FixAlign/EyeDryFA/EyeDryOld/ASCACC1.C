...
int GetNextLongNumber(char* dc,int *i);
float GetNextFloat(char* dc,int *i);
int GetAnswer(FILE *infile,int *ntrials, int *endtime);
int GetCorrect(FILE *infile,int *ntrials);
int GetRespTime(FILE *infile,int *ntrials);
int GetSync(FILE *infile,int *ntrials);
int answers_correct[MaxNoCond];
int answers_n[MaxNoCond];
int tot_answers_correct,tot_answers_n;
int RT_Sum_Cond[MaxNoCond];
int correct;
int ncorrect,n;
/* MAIN PROGRAM */
	...
	for(i=0;i<=maxSencond-minSencond;i++)
		...
		if(answers_n[i] > 0)
			...
			if(answers_correct[i] > 0)
				fprintf(outfile2,"%d,%s,%d,%d,%d,%d,%ld\n",nsub+1,sfile,minSencond+i,answers_correct[i],answers_n[i],100*(answers_correct[i])/answers_n[i],(RT_Sum_Cond[i]/(int)(answers_correct[i])));
			else
