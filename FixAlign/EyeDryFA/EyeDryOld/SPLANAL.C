char version[] = "SPLANAL 11/12/98";
/*...................................................
SPLANAL.C, modification of SEGMENT.C
...
void summarize(int nimin,int nimax,int nscmax,int nsegmax,int ncmin,int ncmax,int senfileq,int nitot,int nctot);
...
if(strlen(buff) != 0)
	...
	mschar = 0;
	printf("Shortest listening time, ms (can be negative; discard if below): ");
	shorttime = atoi(gets(buff));
	printf("Longest listening time, ms (discard if above): ");
...
while((changes = correct(&debug,&qskip,&lookahead,&minq,&maxq,&nimin,&nimax,&nsegmax,&ncmin,&ncmax,&nscmax,&cpos,&ipos,&npos,&dpos,&longtime,&shorttime,&skiptime,&mschar,&nitot,&nctot,&qcpos,&qresppos,&avg,allname,subname,senname))> 0)
	;
...
summarize(nimin,nimax,nscmax,nsegmax,ncmin,ncmax,senfileq,nitot,nctot);
fprintf(printer,"\f\n");
if(allfileq)
	fclose(allbuff);
if(senfileq)
				...
				*(ITEMCOND+ti1) = cptrptr; /* cond no for this subj,this sent */
				for(cnt=1,fpt=1,lpt=0;lpt != endsen+1 ;cnt++)  /* Kludge #1 */
					...
					for(;fpt < lpt;skippos--,fpt++)
						...
						temp = atoi(tbuff);
						temp -= *(REGION_LENGTH + ti3);				/* subtract region duration */
...
void summarize(nimin,nimax,nscmax,nsegmax,ncmin,ncmax,senfileq,nitot,nctot)
int nimin,nimax,nscmax,nsegmax,ncmin,ncmax,senfileq,nitot,nctot;
{
char temp[20];
...
correct(int *debug,int *qskip,int *lookahead,int *minq,int *maxq,int *nimin,int *nimax,int *nsegmax,int *ncmin,int *ncmax,int *nscmax,int *cpos,int *ipos,int *npos,int *dpos,int *longtime,int *shorttime,int *skiptime,int *mschar,int *nitot,int *nctot,int *qcpos,int *qresppos,int *avg,char *allname,char *subname,char *senname)
{
int i,j,k;
char temp[20];
