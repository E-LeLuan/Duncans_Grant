...
int ncmaxl,ncminl;
int last_cond;
int nc,ni,nsc,nsca,corrsc,cval,ival,rtval,scval,pval,position;
int sc[20];
char cstring[80];
int DEBUG;
int vexcept[400];
...
if(strlen(buff) != 0)
	...
	fscanf(control,"%d%d%d%d%d",&cval,&ival,&rtval,&scval,&pval);
	...
	nc = ncmaxl-ncminl+1;			/* addition from CPM */
	printf("What is the largest position(sequence) number you want to look at?");
	pval = atoi(fgets(buff,MAXLINE,stdin));
...
if(strlen(file) != 0)
	...
	fprintf(control,"\n%d %d",ncminl,ncmaxl);
	fprintf(control,"\n%d %d %d %d %d",cval,ival,rtval,scval,pval);
...
printf("\n30: Mean (0) or Median (1): %d  CHANGES ? ",*median); 
...
if(strlen(buff) == 0)
	...
	fprintf(printer,"\nLargest sequence/position examined %4d",pval);
...
while(fgets(buff,MAXLINEX,subbuff) != NULL && strlen(buff) > 1)
	{
	position = atoi(buff+spacebuff(1));		/* get sequence position */
	...
	if(trials[nf].cond >= ncmin && trials[nf].cond <= ncmax && trials[nf].subclass < nsc && trials[nf].item >= nimin && trials[nf].item <= nimax && position > pval)
		{
