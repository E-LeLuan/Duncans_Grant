char version[] = "2/17/2008 for IXS file; old .crn won't work right";

/* kludged to use "RT" to score questionnaire answers, * 100 */
...
printf("\n\nKludged version for questionnaire\nType in '5' for both resp and RT\nit will multiply this response by 100 for means");
printf("\nRemember to set limits = max resp * 100\n");
...
rewind(control);
fprintf(ixsfile,"Subj,Item,Cond,Corr,RT,Resp-1\n");	/*Resp-1 because it's the index of the response */
													/* so in Resp = 1 or 2 it comes out 0 or 1, for prop 2's */
for(i=0;i<ns;i++)
	{
