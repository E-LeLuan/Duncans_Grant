...
/* proportion correct straightened out:
	psubfile: proportion correct, S x S, all Rs, regardless of limits
	mpsbfile: number correct each response, first for only Rs within limits
		and then for all Rs
	printout: mean prop is the mean of each indiv S proportion from psubfile
		(0 if a subject has no observations); broken down by R, two blocks,
		one for only Rs within limits, other for all Rs */
...
if(strlen(buff) != 0)
	...
	printf("MRT will average RTs over several of these.\n   How many do you want to average over? ");
	nsca = atoi(fgets(buff,MAXLINE,stdin));
	printf("MRT will treat several of these as 'correct' responses.\n   How many do you want to treat as correct? ");
	corrsc = atoi(fgets(buff,MAXLINE,stdin));
	printf("MRT will treat the first %d values you specify next as correct,\n   and will average RTs over the first %d of them.\n",corrsc,nsca);
	for(i=0;i<nsc;i++)
		{
	...
	printf("Output file name for subject-by-subject RT totals? ");
	tgets(sfname,stdin);
	if(strlen(sfname) == 0)
		strcpy(sfname,"subfile.dat");
	printf("Output file name for subject RT data separated by response value? ");
	tgets(msfname,stdin);
	if(strlen(msfname) == 0)
		strcpy(msfname,"msubfile.dat");
	printf("Output file name for item-by-item RT totals? ");
	tgets(ifname,stdin);
	if(strlen(ifname) == 0)
		strcpy(ifname,"itmfile.dat");
	printf("Output file name for item RT data separated by response value? ");
	tgets(mifname,stdin);
	if(strlen(mifname) == 0)
		strcpy(mifname,"mitmfile.dat");
	printf("Output file name for subject-by-subject prop. correct totals? ");
	tgets(spname,stdin);
	if(strlen(spname) == 0)
		strcpy(spname,"psubfile.dat");
	printf("Output file name for subject PC data separated by response value? ");
	tgets(mspname,stdin);
	if(strlen(mspname) == 0)
		strcpy(mspname,"mpsbfile.dat");
	printf("Output file name for item-by-item prop. correct totals? ");
	tgets(ipname,stdin);
	if(strlen(ipname) == 0)
		strcpy(ipname,"pitmfile.dat");
	printf("Output file name for item PC data separated by response value? ");
	tgets(mipname,stdin);
	if(strlen(mipname) == 0)
...
if((totmat =(long int *)calloc(nc*nsc,4)) == NULL)
	...
	printf("\ntotmat = %p",totmat);
if((allr_totmat =(long int *)calloc(nc*nsc,4)) == NULL)
	oops("allr_totmat");
else
	printf("\nallr_totmat = %p",allr_totmat);
if((nmat = (int *)calloc(nc*nsc,2)) == NULL)
	oops("nmat");
else
	printf("\nnmat = %p",nmat);
if((allr_nmat = (int *)calloc(nc*nsc,2)) == NULL)
	oops("allr_nmat");
else
	printf("\nallr_nmat = %p",allr_nmat);
...
if((inmat = (int *)calloc(ni*nc*nsc,2)) == NULL)
	...
	printf("\ninmat = %p",inmat);
if((allr_inmat = (int *)calloc(ni*nc*nsc,2)) == NULL)
	oops("allr_inmat");
else
	printf("\nallr_inmat = %p",allr_inmat);
...
for (i=0;i<nf;i++)
	...
	if(trials[i].rt >= cutoff)
		...
		if(last_cond)
			(*(nrejsum + trials[i].lcond - ncminl + (trials[i].item-nimin)*nc))++;
		else
...
if(nrt != 0)
	...
	{
	rts = 0;
	fprintf(printer,"\n!!! NO VALID RTs SUBJECT %s",filename);
	}
	
for(i=0;i<nf;i++)
	if((double)trials[i].rt > rts && trials[i].subclass <= corrsc && trials[i].rt != cutoff && rtsq != 0.0)
		{
		if(last_cond)
			(*(nrejsum + trials[i].lcond - ncminl + (trials[i].item-nimin)*nc))++;
		else
			(*(nrejsum + trials[i].cond - ncmin + (trials[i].item-nimin)*nc))++;
		if(spcon)
			fprintf(printer,"\n sigmacutoff %d %d %d %d %d",trials[i].cond,trials[i].lcond,trials[i].subclass,trials[i].item,trials[i].rt);
		if(truncelim || (int)(rts) < 0)
			trials[i].rt = 0;
		else
			trials[i].rt = (int)(rts);
		}
	...
	*(allr_inmat + imptr) += 1;
	...
	for(i=0,jccpl=1;i<nc;i++,jccpl++)
		...
		for(j=0;j<nsc;j++)
			...
			fprintf(pmsubfile,"%6d%6d",*(nmat+mptr),*(allr_nmat+mptr));
			}
		fprintf(msubfile,"\n");
		fprintf(pmsubfile,"\n");
	...
	for(j=0,jicpl=1;j < nc;j++,jicpl++)
		...
		if(!median)
			...
			for(k=0;k<nsc;k++)
				...
				fprintf(pmitemfile,"%6d%6d",*(inmat+iptr),*(allr_inmat + iptr));
				if(buff[0] == 'y')
...
/*			if(tempr > 0)*/
		...
		if (DEBUG == 3)
			printf("\n   --tempr tot %ld i %d j %d",tempr,i,j);
/*		if(tempr > 0)*/
			{
			*(nsum + j) += 1;
			*(totsum + j) += tempr;
...
for(i=0;i<nc;i++)
	{
	if(*(nsum + i) != 0)
		fprintf(printer,"\nC %3d Mean RT: %6ld   Means each resp:",i+ncminl,*(totsum+i)/(long)(*(nsum+i)));
	else
		fprintf(printer,"\nC %3d Mean RT:  00000    Means each resp:",i+ncminl);
	for(j=0;j<nsc;j++)
		{
		mptr = i*nsc + j;
		if(*(nmat+mptr) != 0)
...
for(i=0;i<ns;i++)
	...
	for(j=0;j<nc;j++)
		...
		for(k=0;k<nsc;k++)
			{
			mptr = j*nsc + k;
			fscanf(pmsubfile,"%d%d",&tempi,&allr_tempi);
			*(totmat+mptr) += (long)(tempi);
			*(allr_totmat + mptr) += (long)(allr_tempi);
			}
...
for(i=0;i<nc;i++)
	{
	fprintf(printer,"\nC %3d Mean PC: %6.3f  # valid Rs by resp:",i+ncminl,((double)(*(totsum+i)/(long)(*(nsum+i))))/1000);
	for(j=0;j<nsc;j++)
		{
		mptr = i*nsc + j;
...
for(i=0;i<nc;i++)
	{
	fprintf(printer,"\nCond %3d               # all Rs by resp:  ",i+ncminl);
	for(j=0;j<nsc;j++)
		{
		mptr = i*nsc + j;
		fprintf(printer,"%6ld",*(allr_totmat+mptr));
		}
	}
