char version[] = "EYEDRY VERSION OF 2/16/2012 (compiled with mingw-GCC)";
/*...................................................
...
2/16/2012: add items x subjects output to saccade matrix option
(vector of from-to occurrences: 1_1 is saccade from region 1 to region 1, 
1_2 from region 1 to region 2, ... 5_3 from region 5 to region 3...)
...
int ti1,ti2,ti3,tiall;                        /* temporary indices into arrays */
...
int *ALL_MATRIX;			/* for sub x item output */
int debug;
...
if(anal != 12 && anal != 17 && anal < 18) /* subj by items file */
	{ /*not pos as func of time nor lengths */
	...
	if(strlen(AllfileName) > 0 && anal != 15)
		{
...
if(strlen(AllfileName) > 2)
	...
	if(debug != 0)
		printf("\nOpening Allfile: AllfileName %s, Allfile %d\n",AllfileName,Allfile);
	}
...
if((I_MATRIX = (int *)(calloc(nitot*nscmax*nregmax*nregmax,sizeof(int))))==NULL)
	oops("I_MATRIX");
if((ALL_MATRIX = (int *)(calloc(nscmax*nitot*nregmax*nregmax,sizeof(int))))==NULL)
	oops("ALL_MATRIX");
}
...
if(strlen(AllfileName) > 2 && anal != 20 && anal != 15)
	{
	if (wide)
		{
...
if(strlen(AllfileName) > 2 && anal == 15)			/* saccade matrix */
	{
	if(debug != 0)
		printf("\nAllfileName = %s nregmax = %d",AllfileName,nregmax);
	fprintf(Allfile,"subj,cond,item");
	for(i = 0; i <nregmax; i++)
		for(j = 0; j < nregmax; j++)
			fprintf(Allfile,",%d_%d",i+1,j+1);			/* matrix header, string of from_to */
	fprintf(Allfile,"\n");
	}
...
/*      printf("\nSubject %d; what is y offset? ",i+1);
	...
	if(debug != 0)
		printf("\nEntering cleararrays, subject %d",i+1);
	cleararrays(nitot,nctot,nscmax,nregmax,i);
	if(debug != 0)
		printf("\nBackFrom cleararrays, subject %d",i+1);
...
if(debug != 0)
	{
	printf("\nEntering sortsubject, subject %d PRESS KEY",i+1);
	getch();
	}
...
if(anal == 15 && strlen(AllfileName) > 2)	/* matrix analysis */
	{
/* write out SxI matrix */
if(debug > 0)
	printf("\nEntering writing of allfile, matrix: Allfile = %d",Allfile);
	for(j = 0,jc=ncmin-1;jc< ncmax;j++,jc++)
		for(k = 0; k < nitot; k++)
			{
			ti1 = j*nitot + k;
			if(*(ITEMCOND + ti1))			/* write only if data */
			{
			fprintf(Allfile,"%d,%d,%d",i+1,jc+1,k+nimin);
			for(l = 0; l < nregmax; l++)
				for(m =0;m < nregmax; m++)
					{
					tiall = j*nregmax*nregmax*nitot + k*nregmax*nregmax + l*nregmax + m;
/*if(debug > 0)
	{
	printf("\ns %d cond %d item %d from %d to %d tiall %d obs %d PRESS KEY",i,jc+1,k+1,l+1,m+1,tiall,(*(ALL_MATRIX+tiall)));
	getch();
	}*/
					fprintf(Allfile,",%d",(*(ALL_MATRIX+tiall)));
					}
			fprintf(Allfile,"\n");
			}
			}
...
else if(anal == 17 && strlen(SubfileName) > 2)	/* landlaunch matrix analysis */
	{
	for(j=0,jc=ncmin-1;jc<ncmax;j++,jc++)
		{
		fprintf(Subfile,"%5d%5d  ",i+1,jc+1);	/* subject and condition number */
...
/*printf("\nregion %d otemptime %d, ftemptime %f, start_region %d, end_region %d",region,otemptime,ftemptime,start_region,end_region);*/
		...
		if(debug EQ 2)
			{
			printf("\nanal 15 or 18: SPTRPTR = %d",sptrptr);
		...
		if(debug >= 2)
			printf("\nFound condition match in anal 15 or 18: icond = %d cptrptr = %d ti1 = %d",icond,cptrptr,ti1);
		if(icond EQ nscmax)
			{
			printf("\nSCREWUP SEARCHING FOR SUBCOND IN ANAL 15 or 18! sen %d cond %d",sptrptr,cptrptr);
			exit(1);
			}
		if(debug EQ 2)
			printf("\n  anal 15 or 18: NS = %d, adjusted cond = %d",ns,cptrptr);
...
/*						if(getchar() == 'q')
					...
					if(anal == 15)	/* matrix of movements */
						{
						if(debug > 0)
							printf("\nGoing to fill_matrix for matrix analysis");
						...
						if(debug > 0)
							printf("\nBack from fill_matrix for matrix analysis");
						}
					else if(anal == 18)
						{
...
if(anal == 15)
	...
	for(i=0;i<nscmax;i++)								/* BE CAREFUL.... opposite of others*/
		for(j = 0; j < nitot; j++)
			for(k=0;k<nregmax;k++)
				for(l=0;l<nregmax;l++)
					{
					tiall = i*nregmax*nregmax*nitot + j*nregmax*nregmax + k*nregmax + l;
					*(ALL_MATRIX+tiall) = 0;
					}
...
if(temptime > longtime && chatter)			/* time longer than set for */
	...
	if(fixation == 0 || (which_matrix == 2 && last_pos < far_fix))		/* filter out saccades from regressions */
		...
		(*(I_MATRIX+ti2))++;
		tiall = curr_cond*nregmax*nregmax*nitot + nitem*nregmax*nregmax +last_pos*nregmax + fix_pos;		/* for sXi output */
if(debug > 0)
	printf("\nIn Fill_matrix, fixation %d, fix_pos %d, last_pos %d,tiall %d,curr_cond %d,nregmax %d,nitot %d, nitem %d ",fixation,fix_pos,last_pos,tiall,curr_cond,nregmax,nitot,nitem);
		(*(ALL_MATRIX + tiall)) = 1;		/* just record 1 or more saccades */
		last_pos = fix_pos;
		}
	if(fix_pos > far_fix)
		far_fix = fix_pos;
