...
mincond = input("What is the smallest experimental condition number? ");
...
while(fgets(tempitem,max_chars,infile) != NULL)
	{
	if(strlen(tempitem) > 2 && PartStringCompare(tempitem,"trial") && (*(tempitem+6) == 'E'))	/* only E trial */
		{
		ttl = tempitem+7;		/* assume cond number starts right after E */
		tsptr = tempstring;
		while(isdigit(*ttl))
			*(tsptr++) = *(ttl++);
		*tsptr = '\0';
		...
		if(*(ttl) == '0' && CCOND >= mincond && CCOND <= maxcond && CITEM >= minsen && CITEM <= maxsen)		/* an experimental nondependent item */
			...
			*(mat_pos+(CCOND-mincond)) = 0;		/* assume start in first column */
			for(xyp=ttl,xyc=1,xval=0;*xyp != '\0';xyp++,xval++)
				{
				while(xval==0 && (*xyp == '\n' || *xyp == LF || *xyp == CR || *xyp == ' '))
					xyp++;			/* skip leading new lines  and spaces*/
