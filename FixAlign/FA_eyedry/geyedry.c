char version[] = "EYEDRY VERSION OF 8/6/02 (with Egan modifications)";
/*...................................................
...
9/90: cumulative position added; start of critical region and then
		fixation position every "cum_grain" (10 or 20) msec for 100 samples
...
1 = first fixation
	option: raw or ms/ch
	option: conditionalize on presence or absence of regression
...
5/27/01: Add upper time cutoffs to first pass and total time (discard trials
where fp or total time is above the cutoff).
...
6/4/04: Patrick Sturt; modified code to compile under gcc:
...
#include "stdlib.h"
#include "math.h"
//#include "alloc.h"  /*DOS library*/
//#include "dos.h"    /*DOS library*/
#include "string.h"
//#include "ctype.h"  /*DOS library*/
//#include "conio.h"  /*DOS library*/
...
long * CUMV; /*cumulative values, cumulated over subjects */
int *	CUMNV;
long *CONDV;                /* indiv S condition values */
int *CONDNV;
long  *CUMCV;
int *CUMCNV;
long  *CUMCVT;
...
int sec_pass(int fixation,int temppos,int temptime,int start_region,int end_region,int shorttime,int longtime,int cptrptr,int sptrptr,int region,int ns,int *past_region,int *in_region,int poke_anal,int region_length,int endsen);
...
void getcontrol(int nimin,int nitot,int ncmin,int nctot,int nregmax,int nscmax);
...
if(tolower(tch[0])=='y')
	{
	  printf("\nSorry, printer output is unavailable in this version\n");
	  exit(1);
	}
**/
//else
...
if(strlen(cfile) != 0)
	...
	printf("What is the screen width in characters? 60, 72, or 80: ");
...
if((NR = (int *)(calloc(nitot*nscmax,sizeof(int))))==NULL)
	oops("NR");
getcontrol(nimin,nitot,ncmin,nctot,nregmax,nscmax);	/* get the control list into C */
fprintf(printer,"***********************************************************\n\n");


while(1)
{
printf("\nWhich analysis?\n  1 = first (or second) fixation\n  2 = first pass time\n  3 = total time\n  4 = first pass regressions out\n  5 = regressions in\n  6 = prob of first-pass fixation (or duration of fix before/after skip/land)\n  7 = number of fixations\n  8 = second pass times\n  9 = eyepoke\n 10 = eyegaze\n 11 = landing position or launch site\n 12 = position as function of time\n 13 = time from entering region to first going PAST it\n 14 = first fixation or saccade after LEAVING region (spillover)\n 15 = saccade matrix (from region to region)\n 16 = total gaze\n 17 = Landing/launch position\n 18 = inhibition of return stuff\n0 TO QUIT: ");
...
if(anal < 4 || anal >= 8)
	...
	if(anal == 1 || anal == 9 || anal == 11 || anal == 12 || anal == 14 || anal == 15 || anal == 16 || anal == 17  || anal == 18)
       ...
       /* conditionalize on regressions */
		if(anal <= 3)
			...
			printf("Conditionalize on presence/absence of regressions? y or n: ");
...
/* also kludge for landing/launch site */
	...
	if(anal == 9)
		...
		printf("What is sample size in msec? Type 10 or 20: ");
		...
		if((x = tolower(*gets(buff))) == 'r')
			...
			printf("\nNot a legal response; setting value to forward-goig only.\n");
		if((poke_anal/2)%2 == 0)
			{					/* spillover durations */
...
if(anal != 12 && anal != 15 && anal != 17 && anal != 18) /* subj by items file */
	{ /*not pos as func of time nor matrix */
...
else		/* inhibition of return analysis */
	{
...
if((NV =  (int *)(calloc(nitot*nscmax*nregmax,sizeof(int))))==NULL)
	oops("NV");
if((CUMV =  (long  *)(calloc(nitot*nscmax*nregmax,sizeof(long ))))==NULL)
	oops("CUMV");
if((CUMNV =  (int *)(calloc(nitot*nscmax*nregmax,sizeof(int))))==NULL)
	oops("CUMNV");
if((CONDV =  (long  *)(calloc(nctot*nregmax,sizeof(long))))==NULL)
	oops("CONDV");
if((CONDNV =  (int *)(calloc(nctot*nregmax,sizeof(int))))==NULL)
	oops("CONDNV");
if((CUMCV =  (long  *)(calloc(nctot*nregmax,sizeof(long))))==NULL)
	oops("CUMCV");
if((CUMCNV =  (int *)(calloc(nctot*nregmax,sizeof(int))))==NULL)
	oops("CUMCNV");
if((CUMCVT = (long  *)(calloc(nctot*nregmax,sizeof(long))))==NULL)
	oops("CUMCVT");
...
/*      printf("\nSubject %d; what is y offset? ",i+1);
	...
	if(sortsubject(i,nimin,nimax,ncmin,ncmax,nscmax,qskip,cpos,ipos,npos,dpos,longtime,shorttime,lookahead,minq,maxq,qcpos,qresppos,nitot,poke_anal,cum_grain,nregmax,nctot,around_end,double_space,StartPrevFix,EndPrevFix) == 0)
...
/* WRITE ITEM BY SUBJECTS INFORMATION */
	if(strlen(AllfileName) > 2)
		for(j = 0; j < nitot; j++)
			...
			{
			ti1 = k*nitot + j;
			fprintf(Allfile,"%6d%6d%6d",i+1,j+1,*(ITEMCOND + ti1));
			for(l = 0; l < nregmax; l++)
				{
				...
				if(mspc || (anal == 9 && poke_anal >= 2 && l != 3))
						{
						if(avg)
							fprintf(Allfile,"%8.2f",(float)((*(V+ti2)))/((float)((*(NV+ti2))))*10.0);
						else
							fprintf(Allfile,"%8.2f",((float)*(V+ti2))/10.0);
						}
					else
						{  /* ~mspec && (anal != 9 (eyepoke) or anal = 9 but poke_anl < 2) */
						if(avg)
							fprintf(Allfile,"%8d",(*(V+ti2))/(*(NV+ti2)));
						else
							fprintf(Allfile,"%8d",*(V+ti2));
						} /* end ~mspec && ... */
					} /* end if * (NC+ti1)) */
				else
					if(anal == 11)
						fprintf(Allfile,"%8d",-1);
					else
						fprintf(Allfile,"%8d",0);
				}  /* end l = 0; l < nregmax; l++ */
			fprintf(Allfile,"\n");
...
while(fgets(buff,MAXLINEX,gdbuff) != NULL)	/* keep going until EOF */
	...
	if(debug EQ 2)
		{
		printf("\n\n NEW SENTENCE sentence %d, cond %d/press key to continue, q to quit.",sptrptr,cptrptr);
		if(getchar() == 'q') 
			exit(1);
		}
	...
	/*major loop*/
		...
		{ /*get data pos and data for any analysis but 15 or 18 */
...
/*end if cond_reg, have checked for regressions so can conditionalize */
	...
	if((lookahead ==  qresp || lookahead == FALSE) && (cond_reg == 0 || got_cond_reg == 1))
			{ /*another major loop */
			ti1 = icond*nitot + ns;
		      ...
		      if(gazeanal)   /* eyepoke, eyegaze */
				...
				if(start_record && tempy >= 0 && tempy < max_y) /* discard fixations with negative */
					...
					if(anal == 1)
						...
						fixation = sec_pass(fixation,temppos,temptime,start_region,end_region,shorttime,longtime,cptrptr,sptrptr,region,ns,&past_region,&in_region,poke_anal,region_length,endsen);
...
/* weird kludge for inhibition of return code, check for > n regressions && no successive regressions */
			...
			for(fixation=0,start_record=FALSE;fixation < endsen && (anal == 15 || !toomanyregressions); fixation++)
				...
				if(tempy >= 0 && tempy < max_y) /* discard fixations with negative */
					...
					if(debug == 2)
						{
						printf("\nin sortsubject for anal 15, 18: found fixation %d temppos %d\n   tempx %d tempy %d temps %d tempe %d",fixation,temppos, tempx,tempy,temps,tempe);
						getchar();			/* patch for Maria Nella */
						}
...
getch();*/
		...
		if(*(NV+ti2) == 0)		/* first fix only */
			...
			if(fixation == 0 && poke_anal == 5)
				...
				else if(poke_anal == 5)				/* previous fixation, obtained from PrevFixOK() */
					{
					*(V+ti2) = temp_fixation;
					*(NV+ti2) = 1;
					fixation = endsen+1;
...
if(temppos >= start_region && temppos < end_region)
	...
	if(temptime > shorttime && temptime <= longtime)
		{
		if(fixation > 1 && StartPrevFix != 0 && *(NV+ti2) == 0)		/* filter on basis of prev fix */
			{
			if(PrevFixOK(start_region,shorttime,StartPrevFix,EndPrevFix,longtime) == 0)
				{
				fixation = endsen+1;
...
if(poke_anal > 2 && debug == 2)
	{
	printf("\nExiting p_init with gotfix %d reg %d fix %d V %d poke_anal %d endsen %d",gotfix,region,fixation,*(V+ti2),poke_anal,endsen);		
	getchar();
	}
return(fixation);
} /*end p_init_fix*/
...
/*  poke_anal = 0 for start of region, 1 for first fix */
		...
		*(TCUMPOS + local_ti3) = temppos-starting_point;
		...
		if(debug == 2)
			{
			printf("\nIn cumulate_position, ns %d, icond %d, starting_point %d",ns,icond,starting_point);
			getchar();
			}
...
/*			tempe = atoi(tbuff);*/
			movenextnumber();	
			if(debug == 2)
				printf("\nIn cumpos, temppos %d, last *TCUMPOS %d, nowtime %d starttime %d",temppos,*(TCUMPOS+last_ti3),nowtime,temps);
			do
				{
				last_ti3 = local_ti3;
...
if(fixation == endsen)
	for(;local_sample<=cum_positions;local_sample++)	/* fill out rest of line */
		{
		local_ti3 += nitot*nscmax;
		*(TCUMPOS + local_ti3) = -999;		/* fill with dummy fake */
		}
return(fixation);
