...
/* Padraic/Scott's mods for Unix/Linux, 29/03/96 & 22/03/2002 :
          removed reference to "stdprn": DOS function (142-6);
	  removed ref to "date" structure (104) - DOS function; 
	  removed other refs to "today" (166-7);
	  commented out #includes of DOS-specific libraries;
          removed textcolor(), textbackground(), normvideo() calls;
          changed getch() call to gets(temp) (line 777);
          changed cprintf to printf
          (lines 665,676,678,686,688,708,715,719,726,1232)
   compile with: gcc eyewash.c -o eyewash */

#include "stdio.h"
#include "math.h"
#include "stdlib.h"
#include "string.h"
/* #include "dos.h" */      /* DOS-specific library */
#include "ctype.h"
/*#include <stat.h>*/		/* uncomment for TC 1.5 */
#include <fcntl.h>
/* #include "mem.h" */      /* DOS-specific library */
/* #include "alloc.h" */    /* DOS-specific library */
/* #include "io.h" */       /* DOS-specific library */
#include "stddef.h"
/* #include "conio.h" */    /* DOS-specific library */
//#include "gpcexpt.h"
...
if((tlbfile = fopen(filename,"r")) == NULL)
  filename[i] = 'T';   /*** Try again with capital letters ***/
 filename[i+1] = 'L'; /*** (added for unix compatibility) ***/
 filename[i+2] = 'B';
...
/*printf("\ntempconds %d tempitems %d",tempconds,tempitems);*/
	if(tempconds >= fcond && tempconds <= lcond && tempitems >= fitem && tempitems <= litem)
		...
		*ts++ = temp;
		...
		if(*ts == EOF)
			{
			printf("\nPROBLEMS READING SENTENCE IN SENFILE, TRIAL %d",itrial+1);
			EXIT(1);
			}
			...
			if(((*(xptr+iobs)<(*(xptr+iobs-1)-20)&&*(yptr+iobs)==*(yptr+iobs-1)) || *(yptr+iobs) != *(yptr+iobs-1) || *(yptr+iobs) < 0 || *(xptr+iobs) <= 0 || which>0) || iobs == 1 || (iobs-lobs >= 10) && iobs < 999)
				...
				if(boundary_expt)
					...
					if((dtrials[CTRIAL].resp2)%2)			/* cross boundary in fixation */
						printf("\nWARNING: EYE SLID ACROSS BOUNDARY DURING APPARENT FIXATION\n");
					}
				do
					{
...
/*							textcolor(BLACK);
							...
							if(tobs >= 0 && tobs < dtrials[CTRIAL].obs)
								if(iobs == tobs)
									printf("\r\n*FIX_%2d_X=%3d_Y=%3d_START=%6u_END=%6u_DUR=%6u_SACCADE_DUR=%4u",tobs+1,*(xptr+tobs),*(yptr+tobs),*(sptr+tobs),*(eptr+tobs),*(eptr+tobs)-*(sptr+tobs),*(sptr+tobs)-*(eptr+tobs-1));
								else
									printf("\r\n FIX_%2d_X=%3d_Y=%3d_START=%6u_END=%6u_DUR=%6u_SACCADE_DUR=%4u",tobs+1,*(xptr+tobs),*(yptr+tobs),*(sptr+tobs),*(eptr+tobs),*(eptr+tobs)-*(sptr+tobs),*(sptr+tobs)-*(eptr+tobs-1));
							}
						else
...
/*					normvideo(); */
					...
					if(which == -1)			/* change whole block */
						...
						txy = input("New Y value for these items? ('ENTER' to cancel) ");
						if(txy != 0)
							{
						...
						printf("New X value (ENTER to keep)? ");
						gets(temp);
						if(strlen(temp) > 0)
							{
							txy = atoi(temp);
						...
						{
						printf("New Y value (ENTER to keep)? ");
						gets(temp);
						if(strlen(temp) > 0)
							{
							txy = atoi(temp);
