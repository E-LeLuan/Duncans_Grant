...
/* Modified for ET-addition F11, get E cond number from 2 back */
...
for(nsub=0;nsub<ns;nsub++)
	...
	for(CTRIAL = 0;CTRIAL < ntrials;CTRIAL++)
		{
/* kludge for ET1-addition */
		twobackcond = lastcond;
		...
		if(twobackcond > maxSencond)
			twobackcond = maxSencond+1;
		else if(twobackcond < minSencond)
			twobackcond = minSencond-1;		/* screen out items outside condition range */
...
/*printf("\nOrder %d trial %d cond %d item %d lastitem %d lastcond %d",dtrials[CTRIAL].ord,CTRIAL+1,dtrials[CTRIAL].cond,dtrials[CTRIAL].item,lastitem,lastcond);*/
		if(cond <= maxcond && item <= maxitem && cond >= mincond && item >= minitem)
			...
			tot_answers_n++;
			n++;
			if(twobackcond >= minSencond && twobackcond <= maxSencond)
				{
				(answers_n[twobackcond-minSencond])++;	/* tabulate by SENTENCE condition,limiting to range*/
				if(correct == 1)
					{
					ncorrect++;
					(answers_correct[twobackcond-minSencond])++;
					RT_Sum_Cond[twobackcond-minSencond] += (endtime-synctime);
					}
				}
/*		printf("\nAnalyzing trial %d cond %d item %d %ld %ld %u",CTRIAL+1,dtrials[CTRIAL].cond,item,endtime,synctime,(unsigned)(endtime-synctime));*/
		fprintf(outfile,"%d,%d,%d,%d,%d,%d,%d\n",cond,item,twobackcond,lastitem,answer,correct,(unsigned)(endtime-synctime));
