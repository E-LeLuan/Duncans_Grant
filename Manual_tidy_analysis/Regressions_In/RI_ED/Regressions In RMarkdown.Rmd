...
library(readr)
RI_ED_batch_corr <- read_csv("RI_ED_batch_corr.csv")
RI_ED_batch_error <- read_csv("RI_ED_batch_error.csv")
...
If we drop the Total_reading_cluster the effect of condition is still there (see below) - suggesting to me that the effect of Total_reading_cluster is separate from the effect of condition.
...
Does Total_reading cluster interact with our condition? No (see below.)
...
After controlling for individual differences participants make significantly fewer regressions in when reading facilitated conditions (%) compared to un-facilitated conditions (%). There is a .308 increase in regressions with each increase in total reading scores. In other words, the higher your reading score (indicative of higher reading ability) the easier it is to integrate contextual information into a mental representation of the scenario encountered, indicated by fewer regressions.  However, it is likely WRMT explains overall reading time differences, but not anything to do with the difference between our experimental conditions - otherwise we'd have seen an interaction effect.
...
#Visualisation
all_data_join %>% 
  ggplot(aes(x = cond, y = R5, colour = cond)) + ggtitle("Regressions In for Critical Region: Question") +
  labs(y = "Regression", x = "Prediction") +
  geom_violin() +
  geom_jitter(alpha = .2, width = .1) +
  stat_summary(fun.data = "mean_cl_boot", colour = "black") +
...
modelR5 <- glmer(R5 ~ cond + (1 | subj) + (1 | item), data = all_data_join, family = "binomial") 
summary(modelR5)
...
```
Better reading scores lead to fewer regressions regardless of condition.
