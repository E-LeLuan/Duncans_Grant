...
library(readr)
TT_ED_batch_corr <- read_csv("Manual_tidy_analysis/Total_time/TT_ED/TT_ED_batch_corr.csv")
TT_ED_batch_error <- read_csv("Manual_tidy_analysis/Total_time/TT_ED/TT_ED_batch_error.csv")
...
#Visualisation
all_data_join %>% 
  ggplot(aes(x = cond, y = R3, colour = cond)) + ggtitle("Total Time for Prediction") +
  labs(y = "Reading time in ms.", x = "Prediction") +
  geom_violin() +
  geom_jitter(alpha = .2, width = .1) +
  stat_summary(fun.data = "mean_cl_boot", colour = "black") +
