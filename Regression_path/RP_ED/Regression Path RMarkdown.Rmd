---
title: "Regression Path Markdown"
author: "Lib"
date: "04/08/2020"
...
library(readr)
RP_ED_batch_corr <- read_csv("RP_ED_batch_corr.csv")
RP_ED_batch_error <- read_csv("RP_ED_batch_error.csv")
...
There is no significant difference between our two coniditons.
...
There are no significant differences between coniditons and no effect of our controlling factors.
...
#Visualisation
all_data_join %>% 
  ggplot(aes(x = cond, y = R5, colour = cond)) + ggtitle("Regression Path for Critical Region: Question") +
  labs(y = "Reading time in ms.", x = "Prediction") +
  geom_violin() +
  geom_jitter(alpha = .2, width = .1) +
  stat_summary(fun.data = "mean_cl_boot", colour = "black") +
