...
#What about correlations between the individual difference measures.WHY WONT THIS WORK UGHHHHHH!
#all_data_join$SRS2_total_score_t <- as.factor(all_data_join$SRS2_total_score_t)
#all_data_join$WRMT_total_reading_score <- as.factor(all_data_join$WRMT_total_reading_score)
#all_data_join$EQ <- as.factor(all_data_join$EQ)

#cor(SRS2_total_score_t, EQ, method = c("pearson", "kendall", "spearman"))
...
#working out maximal model from a program
#maximal_R4 <- lmer(R4 ~ SRS2_total_score_t + EQ + WRMT_total_reading_score + WRMT_WI_raw +
#                     (1 + cond | subj) +  (1 + cond | item) , data = all_data_join)
